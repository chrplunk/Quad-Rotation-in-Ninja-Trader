...
// Includes:
// - BLUE 20–80 zone fill (TradingView-style) via OnRender (drawn behind everything)
// - Quad Rotation background (red/green) with editable colors/opacities
// - Zinger vertical lines (your Pine section 9 logic)
// - +++++ warnings when Stoch4 is PEGGED >=90 or <=10 AND when ABCD "shield" is active
//   (Shield active == Stoch4 has stayed above 90 for AbcdBars90 bars OR below 10 for AbcdBars10 bars)
// Notes:
// - Stoch2/Stoch3 can be hidden visually without breaking logic (slope uses internal series)
...
namespace NinjaTrader.NinjaScript.Indicators
{
	public class QR : Indicator
	{
		...
		// +++++ warnings
		[NinjaScriptProperty]
		[Display(Name = "Show +++++ Warnings", Order = 1, GroupName = "ABCD Markers")]
		public bool ShowPlusWarnings { get; set; }

		[XmlIgnore]
		[Display(Name = "+++++ High Color", Order = 2, GroupName = "ABCD Markers")]
		public Brush PlusHighBrush { get; set; }

		[Browsable(false)]
		public string PlusHighBrushSerialize
		{
			get { return BrushSerialization.ToString(PlusHighBrush); }
			set { PlusHighBrush = BrushSerialization.FromString(value); }
		}

		[XmlIgnore]
		[Display(Name = "+++++ Low Color", Order = 3, GroupName = "ABCD Markers")]
		public Brush PlusLowBrush { get; set; }

		[Browsable(false)]
		public string PlusLowBrushSerialize
		{
			get { return BrushSerialization.ToString(PlusLowBrush); }
			set { PlusLowBrush = BrushSerialization.FromString(value); }
		}
		...
		// Put +++++ where you can actually see them (inside the blue zone)
		private const double PlusHighY = 82.0;   // just above the 80 line
		private const double PlusLowY  = 18.0;   // just below the 20 line
		...
		protected override void OnStateChange()
		{
			...
			{
				Name = "QR";
				Description = "Quad Rotation - 4 Stochastics Overlay (Blue 20–80 zone + +++++ pegged/shield + Zingers).";
				IsOverlay = false;
				...
				PlusHighBrush = Brushes.LimeGreen;
				PlusLowBrush  = Brushes.Red;
		...
		protected override void OnBarUpdate()
		{
			...
			// Use INTERNAL %D series for previous values (so hiding plots doesn't break logic)
			...
			// Pine: barssince(stoch4D <= 90) > AbcdBars90  => our counter resets when s4 <= 90
			// Pine: barssince(stoch4D >= 10) > AbcdBars10  => our counter resets when s4 >= 10
			if (s4 <= 90) barsSinceStoch4Le90 = 0; else barsSinceStoch4Le90++;
			if (s4 >= 10) barsSinceStoch4Ge10 = 0; else barsSinceStoch4Ge10++;

			bool shieldBull = barsSinceStoch4Le90 > AbcdBars90; // long shield (still above 90 for N bars)
			bool shieldBear = barsSinceStoch4Ge10 > AbcdBars10; // short shield (still below 10 for N bars)

			bool peggedHigh = s4 >= 90;
			bool peggedLow  = s4 <= 10;

			// +++++ warnings (pegged OR shield)
			if (ShowPlusWarnings)
			{
				if (peggedHigh || shieldBull)
					Draw.Text(this, "PLUS_HIGH_" + CurrentBar, "+++++", 0, PlusHighY, PlusHighBrush);

				if (peggedLow || shieldBear)
					Draw.Text(this, "PLUS_LOW_" + CurrentBar, "+++++", 0, PlusLowY, PlusLowBrush);
			}

			// SUPER arrows (when MinCount != 4)
		...
		// BLUE 20–80 fill (TradingView-style)
		// Drawn BEFORE base.OnRender so lines/plots/markers are drawn on top.
